...
def get_predictor_name_identifier(identifier):
    # Avoid unnecessary deep copy by making a shallow copy of list attribute only
    class_id = identifier.__class__
    new_identifier = class_id.__new__(class_id)
    new_identifier.__dict__.update(identifier.__dict__)
    if hasattr(new_identifier.parts, "__copy__"):  # defensive in case parts isn't a list
        new_identifier.parts = new_identifier.parts[:]
