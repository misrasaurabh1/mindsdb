...
def fill_query_params(query, params):
    """
    Fills ? parameters in an AST query with values from params.

    params: should be a list; it's consumed (mutated)
    """
    # Optimization: no need for deepcopy unless params will be mutated inside callback (not the case here).
    params = list(params)  # makes a shallow copy just to be safe
