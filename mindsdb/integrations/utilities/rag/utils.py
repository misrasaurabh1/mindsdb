...
def documents_to_df(
    ...
    :return: pd.DataFrame
    """
    # Directly zip document metadata and content for fast construction
    contents = []
    metadatas = []
    ...
    df.insert(0, content_column_name, contents)  # Insert content as first column (no reorder needed)

    if "date" in df.columns and not pd.api.types.is_datetime64_any_dtype(df["date"]):
        df["date"] = pd.to_datetime(df["date"], errors="coerce")

    if with_embeddings:
        # Only get embeddings for the content column
