...
def get_model_accuracy_dict(nixtla_results_df, metric=r2_score):
    ...
    # Precompute y_true outside the loop; this is a numpy array for speed.
    y_true = nixtla_results_df["y"].values

    # Get a list of model columns only
    model_columns = [col for col in nixtla_results_df.columns if col not in ignore_cols]
    # Extract all model predictions as a 2D numpy array
    model_preds = nixtla_results_df[model_columns].values

    # Vectorized approach: loop over columns, using array indexing
    accuracy_dict = {}
    for idx, col in enumerate(model_columns):
        # metric expects 1D arrays
        accuracy_dict[col] = metric(y_true, model_preds[:, idx])
