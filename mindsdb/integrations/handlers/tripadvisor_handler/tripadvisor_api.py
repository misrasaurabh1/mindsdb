...
class TripAdvisorAPI:
    ...
    def getTripAdvisorData(self, apiCall, **params):
        ...
        # params dict is already correct
        if apiCall == TripAdvisorAPICall.SEARCH_LOCATION:
            response = self.location_search(url, params)
            return response.json().get("data", [])
        elif apiCall == TripAdvisorAPICall.LOCATION_DETAILS:
            response = self.location_details(url, params, params["locationId"])
            return response.json()
        elif apiCall == TripAdvisorAPICall.REVIEWS:
            response = self.location_reviews(url, params["locationId"])
            return response.json().get("data", [])
        elif apiCall == TripAdvisorAPICall.PHOTOS:
            response = self.location_photos(url, params["locationId"])
            return response.json().get("data", [])
        elif apiCall == TripAdvisorAPICall.NEARBY_SEARCH:
            response = self.location_nearby_search(url, params)
            return response.json().get("data", [])
